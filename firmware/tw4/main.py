...
pressure_ratioa = 0.775  #ingress to egress ratio to correct the bias I am seeing.
pressure_ratiob = 0.9
I_gain = 0.35
P_gain = 0.15
...
def percent_to_pressure(perc):
    ...
    if pressure > max_pressure:# the actual pressure is divided because the sensor actually allows flow through it 
...
def check_cp_ingress():
    global main_perc
    global pressure_ratioa
    global pressure_ratiob
    print("main_perc:",main_perc)
    if main_perc < 40:
        pressure = percent_to_pressure(main_perc)*pressure_ratiob
    if main_perc >= 40:
        pressure = percent_to_pressure(main_perc)*pressure_ratioa
    return pressure
def check_cp_egress(): # we might want a pressure bias some day so have separate functions to calculate ingress and egress pressures
    ...
    print("main_perc:",main_perc)
    if main_perc < 40:
        pressure = -1*percent_to_pressure(main_perc)/pressure_ratiob
    if main_perc >= 40:
        pressure = -1*percent_to_pressure(main_perc)/pressure_ratioa
    return pressure
def run_pid_ingress():
