...
def save_data_if_mem_full(samples):
    ...
    if gc.mem_free()<30_000:
        outfile = "/thermistor_samples"+str(filename_counter)+".json"  #"/sd/fansamples.txt" to put on sd card if above stuff is done to mount it etc.
        filename_counter += 1
...
def check_temperature(therm_pin):
    ...
    global T0
    global sense_r
    voltage = (therm_pin.read_u16()/65356)*3.3
...
timer = ticks_ms()
samples = []
while ticks_diff(ticks_ms(), timer)<180_000:
