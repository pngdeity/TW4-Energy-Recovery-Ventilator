...
pressure_ratio = 0.75  #ingress to egress ratio to correct the bias I am seeing.
I_gain = 0.35
P_gain = 0.15
...
def check_cp_ingress():
    ...
    pressure = percent_to_pressure(main_perc)*pressure_ratio
    return pressure
def check_cp_egress(): # we might want a pressure bias some day so have separate functions to calculate ingress and egress pressures
    global main_perc
    print("main_perc:",main_perc)
    pressure = -1*percent_to_pressure(main_perc)/pressure_ratio
    return pressure
def run_pid_ingress():
